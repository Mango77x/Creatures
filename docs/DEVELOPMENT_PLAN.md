...
- [x] Step 3 — spine/neck as a physics chain (`Animation.cpp`): replaced the hand-chained `chestAngleLag->spine3->spine2->spine1AngleLag`/`headAngleLag` ExpLerpAngle sequence with a 6-particle body (pinned pelvis, `SpineSeg1-3`, `ChestEnd`, `NeckEnd`). Every particle chases the same bodyYaw-rotated target at its own muscle rate (2.6 near the pelvis up to 14 at `NeckEnd`) instead of each link's target being the previous link's already-lagged value — the "wave" now emerges from the solver's own inertia plus the distance/angle constraints, not from literal chaining. The skull (`SnoutBase`, `HeadTip`, horns/ears/eyes) is deliberately NOT simulated — it's derived rigidly each frame from the real 3D rotation the `ChestEnd->NeckEnd` bone underwent (`RotateVectorShortestArc`), anchored right at `NeckEnd` with no extra flex joint of its own. Took three wrong attempts to get the skull rigid: giving `HeadTip` its own particle with a flexible angle constraint to `SnoutBase` (visibly bent away from the head — nothing physical stops a skull from bending); then giving `SnoutBase` itself its own particle flexibly attached to `NeckEnd` (same problem one joint back); then deriving the skull's orientation from a hand-eased Y-axis-only angle (can't represent a segment that also pitches up/down, so a steep `neckPitch` or a tall/short body made the snout tip the wrong way). **Known unresolved issue, left as-is per explicit user decision (2026-07-29):** the snout (`SnoutBase->HeadTip`) still visibly lags/detaches from the rest of the head during a fast turn — root cause not confirmed (candidates considered: `CreatureMesh.cpp`'s elliptical cross-section for `BoneKind::Head`, whose `PerpendicularBasis` side/up only reads correctly for near-horizontal bones per Phase 9's own note, colliding with the skull now being able to take on much more varied 3D orientations than the old Y-only system ever produced; or a horn/ear mesh mistaken for the snout during debugging — not conclusively distinguished before the user called off further investigation). Revisit if it becomes a blocker; not re-opened otherwise.
- [x] Step 4 — legs with ground contact (`main.cpp`, `Physics.h`/`.cpp`): replaced `IK.cpp`'s exact analytic 2-bone solve (still used only for the DNA rest pose, `Skeleton.cpp`'s `BuildLeg`) with 4 independent per-leg `PhysicsBody`s (Hip pinned each frame to the spine's already-animated hip joint — resolving the gotcha noted below by pinning to a live external anchor instead of a bone/constraint, same pattern the tail/spine already use for their own root — Knee, Foot). The knee gets no muscle target: its position falls out purely from geometry (hip-knee/knee-foot distance constraints + a new constraint type, see next), same as the old law-of-cosines solve. Only the foot has a muscle, pulling toward `Gait.cpp`'s `ComputeFootTarget` (unchanged) at a fast rate (30) so stepping stays snappy instead of mushy. Ground contact via `GroundConstraint` on the foot, terrain height sampled fresh each frame in the same tilted body-local space the old IK already solved in. New **`PoleConstraint`** added to `Physics.h`/`.cpp`: projects the knee back onto the hip->foot/poleDir bend plane every relaxation iteration — without it, a generic `AngleConstraint` only limits how much a joint bends, not which plane it bends in, so the knee could wobble sideways instead of always folding toward local-forward like the old IK guaranteed by construction. `poleDir` is just a fixed direction per leg (currently local-forward for all 4), not itself simulated — a reversed, bird-style knee is a documented one-line flip of that vector, no other change needed, though not implemented/tested yet.
  **Known unresolved issue, user reported before signing off for the night (2026-07-29), not yet investigated:** one knee (front leg, screenshot showed it near the head) bends inverted/backward relative to the others — the debug skeleton showed an oddly wide, doubled-back diamond shape instead of a normal forward knee bend. Leading hypothesis, not confirmed: `poleDir` is currently the same raw local-forward `(0,0,1)` for every leg regardless of `chestAngleLag` — but a front hip's position (`animatedSkeleton.joints[legs[i].hip]`) IS rotated by `chestAngleLag` (see Animation.cpp/main.cpp), and the front foot's gait target is rotated by that same angle too, so the hip->foot axis for a front leg can end up meaningfully rotated away from raw local-forward while the pole constraint still measures its bend plane against the UN-rotated `(0,0,1)` — if that gap gets large enough (sharp chest bend) the fixed pole direction could end up nearly parallel to the hip->foot axis, or on the wrong side of it, flipping which of the two geometrically-valid knee solutions `SolvePoleConstraint` picks. Not verified by actually reading the reported creature's exact chestAngleLag/turn state at the time — first thing to check on resume. If confirmed, the fix is rotating each front leg's `poleDir` by `chestAngleLag` per frame (mirroring how `restFootLocal` already gets rotated for the gait target), not a change to `PoleConstraint` itself.
  Pelvis stays fully kinematic (no leg-to-spine force feedback yet, pitch/roll still computed from the 4 raycast heights same as before) — that two-way coupling is explicitly Step 5's scope, not attempted here.
...
- [x] Muscles: a small force pulling each joint toward its target (DNA rest pose + `Gait.cpp` target + movement direction) — proven on the tail, spine/neck (Step 3), and leg feet (Step 4; leg knees deliberately have none, see Step 4's writeup)
- [x] Ground contact: `GroundConstraint` now used by every leg's foot (Step 4), terrain height sampled fresh each frame in the same tilted body-local space the old IK solved in
- [x] Pole constraints (not originally planned as a separate item, added during Step 4): keeps a knee-like joint confined to a bend plane defined by a fixed reference direction — `AngleConstraint` alone only bounds how much a joint bends, not which plane, which a real knee needs
