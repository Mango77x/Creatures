...
- [ ] Step 4 — legs with ground contact (not started). **Known gotcha to handle explicitly, not assume away:** `Skeleton.cpp` currently has no `Pelvis`/`ChestEnd` -> `Hip` bone (removed on purpose so the hip's own joint cap blends into the torso surface instead of a floating strut — see the comment above `BuildLeg`'s call site). "Every bone becomes a distance constraint" (below) means a leg's `PhysicsBody` built naively from `skeleton.bones` would have nothing tying its hip particle to the spine — the leg would be physically unattached, free to drift off the body under gravity/forces. Step 4 needs to add that attachment constraint explicitly (even if zero visual bone/mesh is drawn for it), not rely on the automatic bone-to-constraint derivation to cover it.
...
- [x] Muscles: a small force pulling each joint toward its target (DNA rest pose + `Gait.cpp` target + movement direction) — proven on the tail and spine/neck (Step 3), still pending for legs (Step 4)
