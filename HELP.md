...
- **Same error kept happening in CI even after adding `pip install jinja2`**: `windows-latest` has several Python installs on the image, and plain `pip install jinja2` can land in a different interpreter than the one CMake's `find_package(Python3)` picks up for GLAD's generator — so the install "succeeds" but the build still can't find `jinja2`. Fixed by pinning one interpreter with `actions/setup-python` and explicitly passing it to CMake via `-DPython3_EXECUTABLE=...`, so the same Python is used for both the `pip install` and the generator.
